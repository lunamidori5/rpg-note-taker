...
def download_file_from_midori_ai(filename, username, reponame, modeltype):
    ...
    folder_path = os.path.join("models", reponame)
    os.makedirs(folder_path, exist_ok=True)
    filepath = os.path.join(folder_path, filename)

    if os.path.exists(filepath):
        print(f"File '{filepath}' already exists.")
        choice = input("Do you want to remove the existing file and redownload? (y/N): ").lower()
        if choice.lower() == 'y':
            os.remove(filepath)
    ...
    try:
        ...
        with open(os.path.join(folder_path, filename), 'wb') as f, tqdm(total=total_size, unit='B', unit_scale=True, desc=f'Downloading Model') as pbar:
